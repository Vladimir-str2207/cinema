...
export class MovieService {
  ...
  async update(
    id: string,
    { title, year, duration, genre, director, poster_path,video_path, description }: UpdateMovieDto,
  ): Promise<Movie | null> {
    myCache.del('movies');
    return await this.movieModel.findByIdAndUpdate(id, { title, year, duration, genre, director, poster_path, video_path, description }, {
      new: true,
    });
  }
